  ...
  test "the duplicate-catalogue strings are translated in ru and et" do
    ...
    for {msgid, ru, et} <- [
          ...
          {"Creates a copy of “%{name}” with all its categories (%{categories}) and items (%{items}). The copy gets the same status and folder, and “(copy)” after its name.",
           ...
           "Loob kataloogist „%{name}“ koopia koos kõigi selle kategooriate (%{categories}) ja toodetega (%{items}). Koopial on sama olek ja kaust ning nime järel „(koopia)“."},
          {"Items keep their SKUs, prices, attributes and suppliers. Items in Deleted, comments, history and links to other systems (such as a shop's product ids) are not copied.",
           "У товаров сохраняются артикулы, цены, атрибуты и поставщики. Товары из удалённых, комментарии, история и связи с другими системами (например, идентификаторы товаров в магазине) не копируются.",
          ...
          {"Images and files are shared with the original, not copied. Removing an image or file from the copy leaves the original alone, but deleting the file itself in Media removes it from both.",
           "Изображения и файлы общие с оригиналом и не копируются. Удаление изображения или файла из копии не затрагивает оригинал, но удаление самого файла в медиатеке убирает его из обоих.",
           "Pildid ja failid on originaaliga ühised, neid ei kopeerita. Pildi või faili eemaldamine koopiast originaali ei mõjuta, kuid faili enda kustutamine meediateegis eemaldab selle mõlemast."},
