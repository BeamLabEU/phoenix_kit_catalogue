...
- **Self-heal for rows whose job never ran** — the queue guard only fires at upload time, so PDFs uploaded while the queue was down (or `extracting` rows orphaned by a worker that died mid-run) would otherwise stay non-terminal forever. `PdfLibrary.requeue_stuck_extractions/1` re-enqueues every `pending` row plus `extracting` rows older than `:stale_after_seconds` (default 900, so a live extraction isn't double-run), capped at 1000 rows/call. It returns honest counts `%{requeued, skipped, failed}` — `skipped` is rows a live job already covers (so "Re-queued N" never takes credit for no-ops). The whole selection is de-duped against live jobs in **one** query (`live_extraction_job_file_uuids/1`) and enqueued with **one** `Oban.insert_all/1`, so a full-cap admin click is a handful of statements, not ~2k per-row round-trips. `retry_extraction/2` (accepts a `%Pdf{}` or `file_uuid`) resets one row to `pending` (clearing `error_message`) and re-enqueues, which is what lets a terminal `failed` row run again; it refuses a SUCCESS-terminal row (`extracted` / `scanned_no_text`) with `{:error, :already_extracted}` unless `force: true` is passed, so a stray caller can't drop a good PDF out of search. Surfaced in the UI: a per-row **Retry** button on `PdfLibraryLive` (failed rows) + the `PdfDetailLive` failed-alert, and a **"Retry stuck"** header action that calls `requeue_stuck_extractions/0`. No cron by design — the heal is visible/operator-driven so a file that reliably kills the worker can't get silently re-hammered.
  - *Perf (tracked):* the live-job dedup filters `oban_jobs` by `args ->> 'file_uuid'`, which has no index → seq-scan on a busy job table. `oban_jobs` is core/host-owned and this repo ships no migrations, so the fix is a partial expression index in a core `phoenix_kit` migration (`ON oban_jobs ((args ->> 'file_uuid')) WHERE worker = 'PhoenixKitCatalogue.Workers.PdfExtractor' AND state IN (...)`) — worth it once the job table grows large. See the comment on `PdfLibrary.extraction_job_pending?/1`.
