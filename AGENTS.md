...
- **Supplier comments** — one `phoenix_kit_comments` thread per item × supplier row (`"catalogue_item_supplier"`), keyed on the thread uuid in `item_supplier_info.metadata["comment_thread_uuid"]` — server-owned, survives price revisions and removal (removal CLOSES the row, never deletes it). Never the CRM company's thread. Hosts register the back-link resolver: `config :phoenix_kit, :comment_resource_handlers, %{"catalogue_item_supplier" => PhoenixKitCatalogue}`. See `Catalogue.SupplierComments`.
